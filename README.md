...
```typescript
scan(options?: { filter?: string[] | undefined; } | undefined) => Promise<{ action: string; }>
```
...
```typescript
scanStop() => Promise<{ action: string; }>
```
...
**Returns:** <code>Promise&lt;{ bluetooth_name: string; mac_address: string; state: string; }&gt;</code>
...
```typescript
onScanFinished() => Promise<Array<{ bluetooth_name: string; mac_address: string; state: string; }>>
```
